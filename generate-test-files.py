...
def gen_test_case_sw_lw(val, address, offset, val_reg, address_reg, dest_reg):
    cycles = 0
    if offset + address > (1 << 22):
        ...
        print(f'sw x{val_reg}, 0x{offset:x}(x{address_reg})')
        cycles += load_constant(address_reg, address)
        print(f'sw x0, 0x{offset:x}(x{address_reg})')
        cycles += load_constant(address_reg, wrapped)
        print(f'lw x{dest_reg}, 0x{offset:x}(x{address_reg})')
        cycles += 3
...
def gen_tests_sw_lw(wrap_arounds = False):
    print(f'## desc = Autogenerated test file for sw/lw (wrap arounds {"allowed" if wrap_arounds else "not allowed"})')
    cycles = 0
    reg = 3
    n_rand = 32 - reg
    print(f'# Random cases (N={n_rand})')
    for _ in range(n_rand):
        val = random_value('int32')
        address = 4*random_value('uint30' if wrap_arounds else 'uint20')
        offset = random_value('uint12')
        cycles += gen_test_case_sw_lw(val, address, offset, 1, 2, reg)
        reg += 1
    print('addi x1, x0, 0')
    print('addi x2, x0, 0')
    cycles += 2
...
def gen_test_case_sb_lb(val, address, offset, val_reg, address_reg, dest_reg):
    ...
    if offset + address > (1 << 22):
        ...
        cycles += load_constant(address_reg, address)
        print(f'sb x0, 0x{offset:x}(x{address_reg})')
        cycles += load_constant(address_reg, wrapped)
...
def gen_tests_sb_lb(wrap_arounds = False):
    print(f'## desc = Autogenerated test file for sb/lb (wrap arounds {"allowed" if wrap_arounds else "not allowed"})')
    cycles = 0
    reg = 3
    n_rand = 32 - reg
    print(f'# Random cases (N={n_rand})')
    for _ in range(n_rand):
        val = random_value('uint8')
        # stupid
        address = random_value('uint31' if wrap_arounds else 'uint22')
        offset = random_value('uint12')
        cycles += gen_test_case_sb_lb(val, address, offset, 1, 2, reg)
        reg += 1
    print('addi x1, x0, 0')
    print('addi x2, x0, 0')
    cycles += 2
...
def gen_test_case_lui(a, a_reg):
    expected = a << 12
    print(f'# testing lui 0x{a:x} = 0x{expected:x}')
    print(f'lui x{a_reg}, 0x{a:x}')
    print(f'## expect[{a_reg}] = 0x{expected:x}')
    return 1

def gen_tests_lui():
    print(f'## desc = Autogenerated test file for lui')
    cycles = 0
    reg = 1
    n_rand = 32 - 1
    print(f'# Random cases (N={n_rand})')
    for _ in range(n_rand):
        val = random_value('uint20')
        cycles += gen_test_case_lui(val, reg)
        reg += 1
    print(f'## cycles = {cycles}')
    return cycles
...
def gen_all_tests():
    ...
    with_stdout_as_file('./riscv-tests/sw_lw_tests.txt', gen_tests_sw_lw)
    with_stdout_as_file('./riscv-tests/sw_lw_wrap_arounds_tests.txt', lambda: gen_tests_sw_lw(wrap_arounds=True))
    with_stdout_as_file('./riscv-tests/sb_lb_tests.txt', gen_tests_sb_lb)
    with_stdout_as_file('./riscv-tests/sb_lb_wrap_arounds_tests.txt', lambda: gen_tests_sb_lb(wrap_arounds=True))
    with_stdout_as_file('./riscv-tests/lui_tests.txt', gen_tests_lui)
